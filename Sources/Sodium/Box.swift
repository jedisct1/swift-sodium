...
public extension Box {
    ...
    /// }
    /// ```
    func seal(message: Bytes, recipientPublicKey: PublicKey, senderSecretKey: SecretKey) -> (authenticatedCipherText: Bytes, nonce: Nonce)? {
        guard recipientPublicKey.count == PublicKeyBytes,
              senderSecretKey.count == SecretKeyBytes
        else { return nil }
     ...
     */
    func seal(message: Bytes, recipientPublicKey: PublicKey, senderSecretKey: SecretKey) -> (authenticatedCipherText: Bytes, nonce: Nonce, mac: MAC)? {
        guard recipientPublicKey.count == PublicKeyBytes,
              senderSecretKey.count == SecretKeyBytes
        else { return nil }
     ...
     */
    func seal(message: Bytes, beforenm: Beforenm) -> Bytes? {
...
public extension Box {
     ...
     */
    func beforenm(recipientPublicKey: PublicKey, senderSecretKey: SecretKey) -> Bytes? {
        var key = Bytes(count: BeforenmBytes)
        guard crypto_box_beforenm(
            &key,
            recipientPublicKey,
            senderSecretKey
        ).exitCode == .SUCCESS else { return nil }
